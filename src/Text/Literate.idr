...
reduce (MkBounded (Any x) _ _ _ :: rest) acc =
  -- newline will always be tokenized as a single token
  if x == "\n"
  then reduce rest ("\n"::acc)
  else reduce rest acc

reduce (MkBounded (CodeLine m src) _ _ _ :: rest) acc =
    if m == trim src
    then reduce rest ("\n"::acc)
...
reduce (MkBounded (CodeBlock l r src) _ _ _ :: rest) acc with (lines src) -- Strip the deliminators surrounding the block.
  reduce (MkBounded (CodeBlock l r src) _ _ _ :: rest) acc | [] = reduce rest acc -- 1
  reduce (MkBounded (CodeBlock l r src) _ _ _ :: rest) acc | (s :: ys) with (snocList ys)
    reduce (MkBounded (CodeBlock l r src) _ _ _ :: rest) acc | (s :: []) | Empty = reduce rest acc -- 2
    reduce (MkBounded (CodeBlock l r src) _ _ _ :: rest) acc | (s :: (srcs ++ [f])) | (Snoc f srcs rec) =
...
extractCode (MkLitStyle delims markers exts) str =
      case lex (rawTokens delims markers) str of
        (toks, (_,"")) => Right (reduce toks Nil)
        (_, (filePos,i))     => Left (MkLitErr filePos i)
